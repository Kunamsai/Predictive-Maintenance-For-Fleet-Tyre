...
Data['Days In Use'] = (datetime.now() - pd.to_datetime(data['Installation Date'])).dt.days data['Total Kilometers'] = data['Days In Use'] * avg_km_per_day

#calculate condition score to use as target for classification model

def calculate_condition_score(psi, depth, temp): psi_factor = 1 if 90 <= psi <= 110 else 0 depth_factor = 1 if depth > 6 else 0.5 if 3 <= depth <= 6 else 0
...
data['ConditionScore']=data.apply(lambda row: calculate_condition_score(row[‘PSI’],row[‘TyreDepth(mm)’] , row['Temperature (°C)']), axis=1) data['Condition'] = data['Condition Score'].apply(lambda score: "Good" if score == 3 else "Average" if score >= 2 else "Bad")

# Add 'Remaining Kilometers' if it doesn’t exist by calculating based on expected lifetime if 'Remaining Kilometers' not in data.columns:
...
print(f"Condition Model Accuracy: {accuracy_score(y_condition_test, condition_preds)}")
print(f"Remaining KM Model RMSE: {np.sqrt(mean_squared_error(y_remaining_km_test, remaining_km_preds))}")
